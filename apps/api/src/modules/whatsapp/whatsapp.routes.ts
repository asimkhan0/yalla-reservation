...
export async function whatsappRoutes(fastify: FastifyInstance) {
    ...
    fastify.get(
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            ...
            console.log(
                `[Meta Webhook] GET (Verification) request for restaurant: ${restaurantId}`,
            );
            ...
            if (mode && token) {
                ...
                if (config && config.provider === 'meta' && config.webhookVerifyToken === token) {
                    ...
                    console.warn(
                        `[Meta Webhook] Verification FAILED for ${restaurant.name}. Expected token: ${config?.webhookVerifyToken}, received: ${token}`,
                    );
            ...
            try {
                ...
                if (body.object) {
                    if (
                        ...
                        console.log(
                            `[Meta Webhook] Received non-message event (status/read/etc) for ${restaurant.name}`,
                        );
                    }
                ...
                console.error(
                    `[Meta Webhook] Error processing webhook for ${restaurantId}:`,
                    error.message,
                );
                request.log.error(error);
                return reply.status(200).send('EVENT_RECEIVED');
